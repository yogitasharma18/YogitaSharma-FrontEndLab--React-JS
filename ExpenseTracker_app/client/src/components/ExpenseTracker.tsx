...
const ExpenseTracker = () => {
    ...
    let amtByRamesh = totalByPayee("Ramesh");
    let amtByRahul = totalByPayee("Rahul");

    const amountToBePaid = () => {
        let amount = 0;

        amount =  Math.abs(amtByRamesh - amtByRahul)/2;
        return amount;
    }

    const whoToPay = () => {
        let payeeName;
        if(amtByRamesh > amtByRahul){
            payeeName = "Rahul has to pay";
        }
        else {
            payeeName = "Ramesh has to pay";
        }
        return payeeName;
    }
    ...
    const addExpense = async ( event : FormEvent<HTMLFormElement> ) => {
        ...
        const expense = {
            ...
            setDate: "2022-12-01" as string
        } as Omit<IItem, 'id'>;

        const updatedItem = await addItem( expense );
        // Passing a new array to setItems(copying/spreading the items and updating them)
        setItems(
            [
                ...items,
                updatedItem
